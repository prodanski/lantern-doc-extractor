...
def query_and_find_best_answer(image, question, best_score, best_answer, best_page):

    out = qa(question=question, image=image)[0]
    ...
    if out["score"] > best_score:
        ...
        best_page = best_page
    return best_score, best_answer, best_page
...
def ask_doc_questions(path, questions, terms, verbose=0, progress_callback=None):
    ...
    for i, (q_key, q_text) in enumerate(questions.items()):
            ...
            for page_idx in primary_pages:
                ...
                best_score, best_answer, best_page = query_and_find_best_answer(images[page_idx], q_text, best_score, best_answer, page_idx)
                if best_score > 0.98:
